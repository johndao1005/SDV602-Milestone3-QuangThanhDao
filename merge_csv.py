...
def mergeFiles(target,dataview):
    ...
    if target=="":
        ...
        showerror("Error","There are change to database please update before upload the new data")
    else:
        dataControl = Model()
        dataControl.upload(target)
        showinfo("Upload success","The data is uploaded and will be loaded for viewing")
        dataview.loadDES()
